...
#
# Bluetooth speaker agent for headless Raspberry Pi
# Based on https://github.com/fdanis-oss/pw_wp_bluetooth_rpi_speaker
#
# Makes the Pi permanently discoverable and automatically accepts
# A2DP (audio) and AVRCP (remote control) Bluetooth connections
...
class Agent(dbus.service.Object):
    ...
    def signal_handler(self, *args, **kwargs):
        ...
        if not self.remote_device and connected is True:
            self.remote_device = path
            print("{} connected".format(path))
        elif path == self.remote_device and connected is False:
            self.remote_device = None
    ...
    def AuthorizeService(self, device, uuid):
        if self.remote_device and self.remote_device != device:
            print("%s tried to connect while %s already connected"
                  % (device, self.remote_device))
            raise Rejected("Connection rejected by user")

        # Always authorize A2DP and AVRCP connections
...
def start_speaker_agent():
    ...
    # a 3 min timeout. Set it as always discoverable.
...
if __name__ == '__main__':
    ...
    options.add_argument(
        "--single-connection",
        action='store_true',
        help="Allow only one connection at a time"
    )
    ...
    # Watch for BlueZ appearing on the bus (e.g. after a restart)
    bus.add_signal_receiver(
        nameownerchanged_handler,
